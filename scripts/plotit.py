...
model = YOLO("ultralytics/yolov8m-pose.pt")

results = model(img,stream=False)
for result in results:
    kpnts = np.squeeze(result.keypoints.data.numpy())
    #print(kpnts,type(kpnts),kpnts.shape,kpnts.dtype)
    _img = draw_pose(img,kpnts[:,0:2])
    print(kpnts[:,2])
    plt.imshow(_img[:,:,::-1])
    plt.show()